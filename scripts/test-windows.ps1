<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for memora.

.DESCRIPTION
    Builds the product binary (build/c/memora.exe) if it is not
    already present, then runs the deterministic Windows integration tests under
    tests/windows/ against a real memora.exe (real stdio / CLI /
...
.PARAMETER Binary
    Path to an existing memora.exe. If omitted, the script builds it
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\memora.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\memora.exe"
