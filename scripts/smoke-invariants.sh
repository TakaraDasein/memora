...
#
# A comprehensive, fast, portable smoke battery for the memora
...
#
# Usage:
#   scripts/smoke-invariants.sh <binary>        # e.g. build/c/memora(.exe)
#
# Portability notes:
...
inv_help() {
    ...
    if printf '%s' "$RB_OUT" | grep -qiE 'usage|memora'; then
        pass "help"
    else
