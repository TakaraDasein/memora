# memora-mcp setup script (Windows)
...
$BinaryName = "memora-mcp"
$InstallDir = Join-Path $env:LOCALAPPDATA "memora-mcp"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use memora-mcp? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["memora-mcp"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "memora-mcp" = $McpConfig } }
...
Write-Host ""
Write-Host "memora-mcp installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/memora-mcp"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/memora-mcp $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/memora-mcp/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "memora-mcp-setup" }
    ...
    $asset = "memora-mcp-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\memora-mcp`"  # graph database"
}
