...
$BinaryName = "memora"
$InstallDir = Join-Path $env:LOCALAPPDATA "memora"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use memora? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["memora"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "memora" = $McpConfig } }
...
Write-Host ""
Write-Host "memora installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/memora"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/memora $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/memora/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "memora-setup" }
    ...
    $asset = "memora-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\memora`"  # graph database"
}
