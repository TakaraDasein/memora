  ...
  return (
    <div className="text-center" role="status" aria-live="polite">
      ...
      <p className="text-muted-foreground text-sm mt-4">
        {receiving ? "Recibiendo grafo" : "Calculando layout"} — hasta{" "}
        {nodeBudget.toLocaleString("en-US")} nodos
      </p>
      <p className="text-muted-foreground/60 text-xs font-mono mt-1 h-4">
        {receiving
          ? progress.totalBytes
            ? `${formatMegabytes(progress.receivedBytes)} de ${formatMegabytes(progress.totalBytes)} MB`
            : `${formatMegabytes(progress.receivedBytes)} MB recibidos`
          : " "}
      </p>
    </div>
  );
