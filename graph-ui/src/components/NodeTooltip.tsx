...
export function NodeTooltip({ node }: NodeTooltipProps) {
  return (
    ...
    >
      <div className="bg-[#111]/95 backdrop-blur border border-white/20 rounded-lg px-3 py-2 text-xs whitespace-nowrap shadow-xl max-w-[350px]">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            ...
            {node.in_calls !== undefined && (
              <span className="text-white/25">
                · {node.in_calls} llamada{node.in_calls === 1 ? "" : "s"}
              </span>
            )}
          </div>
        )}
        <p className="text-white/20 mt-1 text-[10px]">clic para ver código →</p>
      </div>
    </Html>
  );
}
