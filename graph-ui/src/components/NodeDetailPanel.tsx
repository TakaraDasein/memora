...
export function NodeDetailPanel({
  ...
  const loadCode = async () => {
    ...
    try {
      ...
      setCode(res.source ?? "(código no disponible)");
    } catch (e) {
      setCodeError(e instanceof Error ? e.message : "Error al cargar el código");
    } finally {
      setCodeLoading(false);
    }
  };
  ...
  return (
    <div className="w-full bg-card flex flex-col h-full min-h-0 overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 border-b border-border">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <button onClick={onClose} className="text-muted-foreground/40 hover:text-foreground transition-colors text-[16px] leading-none p-1 shrink-0">×</button>
        </div>

        {node.file_path && (
          <p className="text-[11px] text-muted-foreground/60 font-mono mt-2 break-all leading-relaxed">
            {node.file_path}
            {node.start_line ? (
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2.5">
          {canFetchCode && (
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-primary/10 text-primary text-[11px] font-medium hover:bg-primary/20 transition-colors disabled:opacity-50"
            >
              {codeLoading ? "Cargando…" : code ? "Ocultar código" : "Ver código"}
            </button>
          )}
          {ghUrl && (
            <a
              ...
              className="px-2.5 py-1 rounded-md bg-muted/50 text-muted-foreground text-[11px] font-medium hover:bg-muted hover:text-foreground transition-colors"
            >
              Abrir en GitHub ↗
            </a>
          )}
        ...
        {codeError && <p className="text-[11px] text-destructive/80 mt-2">{codeError}</p>}
        {code && (
          <pre className="mt-2 max-h-[300px] overflow-auto rounded-md bg-muted/80 border border-border p-2.5 text-[10.5px] leading-relaxed font-mono text-foreground/80 whitespace-pre">
            {code}
          </pre>
        )}
        ...
        <div className="flex gap-5 mt-3">
          {[
            { label: "Salida", value: outbound.length, color: "text-primary" },
            { label: "Entrada", value: inbound.length, color: "text-accent" },
            { label: "Total", value: connections.length, color: "text-foreground" },
          ].map((s) => (
            <div key={s.label}>
              <p className="text-[9px] text-muted-foreground/60 uppercase tracking-widest">{s.label}</p>
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="px-4 py-3 space-y-4">
          {outbound.length > 0 && (
            <ConnectionSection title="Referencias" count={outbound.length} icon="→" groups={groupByType(outbound)} onNavigate={onNavigate} />
          )}
          {inbound.length > 0 && (
            <ConnectionSection title="Referido por" count={inbound.length} icon="←" groups={groupByType(inbound)} onNavigate={onNavigate} />
          )}
          {connections.length === 0 && (
            <p className="text-[12px] text-muted-foreground/40 text-center py-8">Sin conexiones</p>
          )}
        </div>
      </ScrollArea>
    </div>
  ...
  return (
    <div>
      <p className="text-[11px] font-medium text-muted-foreground mb-2">
        {title} <span className="text-muted-foreground/40">({count})</span>
      </p>
      {groups.map(([type, conns]) => (
        <div key={type} className="mb-2">
          <p className="text-[9px] text-muted-foreground/50 uppercase tracking-wider mb-1 font-medium">
          ...
          <div className="space-y-px">
            {conns.slice(0, 25).map((c, i) => (
              <button
                ...
                className="flex items-center gap-1.5 w-full text-left px-2 py-[4px] rounded-md hover:bg-muted/50 text-[11px] transition-colors group"
              >
                <span className="text-muted-foreground/40 text-[10px] group-hover:text-muted-foreground">{icon}</span>
                <span className="w-[5px] h-[5px] rounded-full shrink-0" style={{ backgroundColor: colorForLabel(c.node.label) }} />
                <span className="text-foreground/60 group-hover:text-foreground truncate">{c.node.name}</span>
                <span className="text-muted-foreground/30 ml-auto text-[10px] shrink-0">{c.node.label}</span>
              </button>
            ))}
            {conns.length > 25 && (
              <p className="text-[10px] text-muted-foreground/40 px-2 py-1">+{conns.length - 25} más</p>
            )}
          </div>
        </div>
      ))}
