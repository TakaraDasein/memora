  ...
  return (
    <div className="flex-1 rounded-xl border border-border bg-card p-4">
      ...
      <p className={`text-[20px] font-semibold tabular-nums ${color}`}>
        {value.toFixed(1)}<span className="text-[11px] text-foreground/40 ml-1">{unit}</span>
      </p>
      <div className="mt-2 h-1.5 rounded-full bg-muted overflow-hidden">
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{ width: `${pct}%`, backgroundColor: pct > 80 ? "#b91c1c" : pct > 50 ? "#a3a3a3" : "#333333" }}
        />
      </div>
    </div>
  );
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${proc.is_self ? "bg-primary animate-pulse" : "bg-green-500"}`} />
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="px-4 py-2 border-b border-border">
        ...
        <span className="text-[10px] text-foreground/15 ml-2">{lines.length} líneas</span>
      </div>
            ...
            lines.map((line, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`py-[1px] ${
                    isErr ? "text-red-500/80" : isWarn ? "text-foreground/60" : "text-foreground/40"
                  }`}
                >
                  {line}
                </div>
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-8 max-w-4xl mx-auto">
        {/* Hero section */}
        <div className="flex flex-col items-center text-center mb-10 pb-6 border-b border-border">
          <img src="/v1tr.png" alt="Memora" className="w-60 h-auto mb-6" />
          <h1 className="text-2xl font-semibold text-foreground/90">Memora</h1>
          <p className="text-[13px] text-muted-foreground/60 mt-1">Grafo de conocimiento del código fuente</p>
        </div>
