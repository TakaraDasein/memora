# memora — Evaluation Plan (159 Languages)
...
Measure how well `memora`'s structured knowledge-graph queries answer real developer
...
for each language L in order (skip if manifest says done):
  ...
  8. DELETE L's index from ~/.cache/memora so the next language starts clean,
...
for lang in $ALL_LANGS; do                      # ALL_LANGS = full 159-name list (§2 / §8)
  ...
  t0=$(now_ms)
  scripts/benchmark-index.sh ~/.local/bin/memora "$lang" /tmp/bench/"$lang" /tmp/eval-results
...
#### Challenge Review: EVALUATION_PLAN.md — memora 159-Language Benchmark
