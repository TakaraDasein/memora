...
def _validate_url_scheme(url: str) -> None:
    ...
    if scheme not in _ALLOWED_SCHEMES:
        sys.exit(
            f"memora-mcp: refusing to fetch non-https URL "
...
def _safe_extract_tar(tf, dest: str) -> None:
    ...
    for member in tf.getmembers():
        if member.issym() or member.islnk():
            sys.exit(
                f"memora-mcp: refusing unsafe tar entry "
...
def _safe_extract_zip(zf, dest: str) -> None:
    ...
    for name in zf.namelist():
        ...
        if not (member_abs == dest_abs or member_abs.startswith(dest_abs + os.sep)):
            sys.exit(
                f"memora-mcp: refusing unsafe zip entry "
        ...
        with open(tmp_path) as f:
            for line in f:
                if archive_name in line:
                    ...
                    if expected != actual:
                        sys.exit(
                            f"memora-mcp: CHECKSUM MISMATCH for {archive_name}\n"
                            f"  expected: {expected}\n"
                            f"  actual:   {actual}"
                        )
                    print("memora-mcp: checksum verified.", file=sys.stderr)
                    break
    except SystemExit:
        raise
    except Exception:
...
def _os_name() -> str:
    ...
    if p == "win32":
        return "windows"
    sys.exit(f"memora-mcp: unsupported platform: {p}")
...
def _arch() -> str:
    ...
    if m in ("x86_64", "amd64"):
        return "amd64"
    sys.exit(f"memora-mcp: unsupported architecture: {m}")
...
def _cache_dir() -> Path:
    ...
    return base / "memora-mcp"


def _bin_path(version: str) -> Path:
    name = "memora-mcp.exe" if sys.platform == "win32" else "memora-mcp"
...
def _download(version: str) -> Path:
    ...
    archive = f"memora-mcp-{ui}{os_name}-{arch}{variant}.{ext}"
    ...
    print(
        f"memora-mcp: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            sys.exit(
                f"memora-mcp: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = "memora-mcp.exe" if os_name == "windows" else "memora-mcp"
        extracted = os.path.join(tmp, bin_name)
        if not os.path.exists(extracted):
            sys.exit("memora-mcp: binary not found after extraction")
