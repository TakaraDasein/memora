...
const binName = isWindows ? 'memora-mcp.exe' : 'memora-mcp';
const binPath = path.join(__dirname, 'bin', binName);

if (!fs.existsSync(binPath)) {
  // Binary missing — try running the install script (handles --ignore-scripts case)
  process.stderr.write('memora-mcp: binary not found, downloading...\n');
  ...
  if (installResult.status !== 0 || !fs.existsSync(binPath)) {
    process.stderr.write(
      'memora-mcp: download failed.\n' +
      'Try reinstalling: npm install -g memora-mcp\n'
    );
    process.exit(1);
  }
}
...
if (result.error) {
  process.stderr.write(`memora-mcp: ${result.error.message}\n`);
  process.exit(1);
}
