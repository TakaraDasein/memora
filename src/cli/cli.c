 ...
 *
 * Port of Go cmd/memora/ install/update logic.
...
#define TAR_BINARY_NAME "memora"
#define TAR_BINARY_NAME_LEN 6
...
static const char codex_instructions_content[] =
    ...
    "\n"
    "This project uses memora to maintain a knowledge graph of the codebase.\n"
...
bool cbm_remove_old_monolithic_skill(const char *skills_dir, bool dry_run) {
    ...
    char old_path[CLI_BUF_1K];
    snprintf(old_path, sizeof(old_path), "%s/memora", skills_dir);
...
static const char CBM_DEFAULT_MCP_SERVER_NAME[] = "memora";
static const char CBM_ANALYSIS_MCP_SERVER_NAME[] = "memora-analysis";
static const char CBM_SCOUT_MCP_SERVER_NAME[] = "memora-scout";
...
static bool cbm_json_mcp_owned_command(const char *command, const char *expected_binary) {
    ...
    return strcmp(command, "memora") == 0 ||
           strcmp(command, "memora.exe") == 0;
}
...
static const char legacy_augment_verify_agent_content[] =
    ...
    "graph.\n"
    "---\n"
    "Use memora for structural discovery. Start with search_graph, continue with "
...
static const char legacy_gemini_verify_agent_content[] =
    ...
    "  - read_file\n"
    "  - grep_search\n"
    "  - mcp_memora_search_graph\n"
    "  - mcp_memora_trace_path\n"
    "  - mcp_memora_get_code_snippet\n"
    "  - mcp_memora_query_graph\n"
    "  - mcp_memora_get_architecture\n"
    "  - mcp_memora_search_code\n"
    "  - mcp_memora_get_graph_schema\n"
    "  - mcp_memora_list_projects\n"
    "  - mcp_memora_index_status\n"
    "  - mcp_memora_detect_changes\n"
    "---\n"
    "Use memora for structural discovery. Start with search_graph, continue with "
    "trace_path, and retrieve exact definitions with get_code_snippet. Use query_graph or "
...
static const char legacy_claude_verify_agent_content[] =
    ...
    "  - mcp__memora__trace_path\n"
    "  - mcp__memora__get_code_snippet\n"
    "  - mcp__memora__query_graph\n"
    "  - mcp__memora__get_architecture\n"
    "  - mcp__memora__search_code\n"
    "  - mcp__memora__get_graph_schema\n"
    "  - mcp__memora__list_projects\n"
    "  - mcp__memora__index_status\n"
    "  - mcp__memora__detect_changes\n"
    "mcpServers: [memora]\n"
    "permissionMode: plan\n"
...
static const char legacy_copilot_verify_agent_content[] =
    ...
    "  - memora/search_graph\n"
    "  - memora/trace_path\n"
    "  - memora/get_code_snippet\n"
    "  - memora/get_graph_schema\n"
    "  - memora/get_architecture\n"
    "  - memora/search_code\n"
    "  - memora/query_graph\n"
    "  - memora/list_projects\n"
    "  - memora/index_status\n"
    "  - memora/detect_changes\n"
    "---\n" LEGACY_CBM_GRAPH_PROFILE_GUIDANCE;
...
static const char legacy_kilo_verify_agent_content[] =
    ...
    "permission:\n"
    "  \"*\": deny\n"
    "  \"memora_search_graph\": ask\n"
    "  \"memora_trace_path\": ask\n"
    "  \"memora_get_code_snippet\": ask\n"
    "  \"memora_query_graph\": ask\n"
    "  \"memora_get_architecture\": ask\n"
    "  \"memora_search_code\": ask\n"
    "  \"memora_get_graph_schema\": ask\n"
    "  \"memora_list_projects\": ask\n"
    "  \"memora_index_status\": ask\n"
    "  \"memora_detect_changes\": ask\n"
    "---\n"
...
static const char legacy_vibe_verify_agent_content[] =
    ...
    "enabled_tools = [\"memora_search_graph\", "
    "\"memora_trace_path\", \"memora_get_code_snippet\", "
    "\"memora_query_graph\", \"memora_get_architecture\", "
    "\"memora_search_code\", \"memora_get_graph_schema\", "
    "\"memora_list_projects\", \"memora_index_status\", "
    "\"memora_detect_changes\"]\n";
...
static char *cbm_build_legacy_kiro_verify_agent_content(const char *binary_path) {
    ...
    bool ok =
        ...
        yyjson_mut_obj_add_str(
            doc, root, "prompt",
            "Use memora for structural discovery. Start with search_graph, use "
        ...
        yyjson_mut_arr_add_str(doc, tools, "@memora/search_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/trace_path") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/get_code_snippet") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/query_graph") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/get_architecture") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/search_code") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/get_graph_schema") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/list_projects") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/index_status") &&
        yyjson_mut_arr_add_str(doc, tools, "@memora/detect_changes") &&
...
static const char legacy_codebuddy_verify_agent_content[] =
    ...
    "tools: mcp__memora__search_graph,mcp__memora__trace_path,"
    "mcp__memora__get_code_snippet,mcp__memora__query_graph,"
    "mcp__memora__get_architecture,mcp__memora__search_code,"
    "mcp__memora__get_graph_schema\n"
    "model: inherit\n"
    "permissionMode: plan\n"
    "skills: codebase-memory\n"
 ...
 * exact same discovery priority, expressed as runnable `memora
...
static const char aider_instructions_content[] =
    ...
    "   memora cli search_graph "
    "'{\"project\":\"<name>\",\"name_pattern\":\".*Foo.*\"}'\n"
    "2. Who calls X / what does X call:\n"
    "   memora cli trace_path "
    "'{\"project\":\"<name>\",\"function_name\":\"Foo\",\"direction\":\"both\"}'\n"
    "3. Read a specific function/class:\n"
    "   memora cli get_code_snippet "
    "'{\"project\":\"<name>\",\"qualified_name\":\"<qn>\"}'\n"
    "4. Complex patterns (Cypher):\n"
    "   memora cli query_graph '{\"project\":\"<name>\",\"query\":\"MATCH ...\"}'\n"
    "5. Project overview:\n"
    "   memora cli get_architecture '{\"project\":\"<name>\"}'\n"
    "\n"
    "First use in a repo: memora cli index_repository '{\"repo_path\":\"<abs "
    "path>\"}'\n"
    "List indexed projects (for <name>): memora cli list_projects '{}'\n"
    "\n"
    "## When to fall back to grep/glob\n"
    "- Searching for string literals, error messages, config values\n"
    "- Searching non-code files (Dockerfiles, shell scripts, configs)\n"
...
#define CMM_MARKER_START "<!-- memora:start -->"
#define CMM_MARKER_END "<!-- memora:end -->"
...
#define CODEX_CMM_TABLE "mcp_servers.memora"
#define CODEX_CMM_SECTION "[" CODEX_CMM_TABLE "]"
#define CODEX_MCP_BEGIN "# >>> memora MCP >>>"
#define CODEX_MCP_END "# <<< memora MCP <<<"
...
#define CODEX_HOOK_BEGIN "# >>> memora SessionStart >>>"
#define CODEX_HOOK_END "# <<< memora SessionStart <<<"
...
int cbm_upsert_codex_hooks(const char *config_path) {
    return cbm_upsert_codex_hooks_command(config_path, "memora hook-augment",
                                          "memora hook-augment");
}
...
static int cbm_upsert_yaml_stdio_mcp(const char *binary_path, const char *config_path,
    ...
    return cbm_yaml_upsert_owned_mapping_entry(config_path, section_key, "memora",
...
static int cbm_remove_yaml_stdio_mcp(const char *binary_path, const char *config_path,
    ...
    return cbm_yaml_remove_owned_mapping_entry(config_path, section_key, "memora",
                                               block);
}
...
#define KIMI_HOOK_BEGIN "# >>> memora Kimi UserPromptSubmit >>>"
#define KIMI_HOOK_END "# <<< memora Kimi UserPromptSubmit <<<"
...
#define CMM_HERMES_HOOK_ID "memora"
...
static int cbm_build_augment_session_script(const char *binary_path, char *script,
    ...
    int written = snprintf(script, script_size,
                           "# SessionStart adapter installed by memora.\n"
                           "$bin = %s\n"
...
static int cbm_build_augment_coverage_script(const char *binary_path, char *script,
    ...
    int written = snprintf(script, script_size,
                           "# PostToolUse view adapter installed by memora.\n"
                           "$bin = %s\n"
                           "if (-not (Test-Path -LiteralPath $bin -PathType Leaf)) { exit 0 }\n"
...
static int cbm_build_cline_context_script(const char *binary_path, const char *event, char *script,
    ...
    int written = snprintf(script, script_size,
                           "# Cline %s context adapter installed by memora.\n"
                           "$bin = %s\n"
                           "if (-not (Test-Path -LiteralPath $bin -PathType Leaf)) { exit 0 }\n"
...
static const char cmm_session_script_prefix[] =
    "#!/usr/bin/env bash\n"
    "# SessionStart context adapter installed by memora.\n"
    "# Fail-open: it never blocks or logs hook/prompt content.\n"
    "BIN=";

static const char cmm_subagent_script_prefix[] =
    "#!/usr/bin/env bash\n"
    "# SubagentStart context adapter installed by memora.\n"
    "# Fail-open: it never blocks or logs hook/prompt content.\n"
    "BIN=";
...
#define GEMINI_HOOK_COMMAND                                                            \
    ...
    "memora search_graph, trace_path, and get_code_snippet over grep or " \
    "file search.'}}))\""
static const char *const cmm_gemini_released_hook_commands[] = {
    "echo 'Reminder: prefer memora search_graph/trace_path/get_code_snippet over "
    "grep/file search for code discovery.' >&2",
    "echo 'Reminder: prefer memora search_graph/trace_call_path/get_code_snippet "
...
#define GEMINI_SESSION_COMMAND                                                          \
    ...
    "memora search_graph, trace_path, get_code_snippet, query_graph, and " \
    "search_code; run index_repository first when needed.'}}))\""
static const char *const cmm_gemini_released_session_commands[] = {
    "echo \"Code discovery: prefer memora (search_graph, trace_path, "
...
unsigned char *cbm_extract_binary_from_targz(const unsigned char *data, int data_len,
    ...
    /* Parse tar: find entry starting with "memora" */
    size_t pos = 0;
...
unsigned char *cbm_extract_binary_from_zip(const unsigned char *data, int data_len, int *out_len) {
    ...
    *out_len = 0;
    ...
    while (pos + ZIP_HDR_SZ <= data_len) {
        ...
        if (strcmp(basename, "memora") == 0 ||
            strcmp(basename, "memora.exe") == 0) {
...
int cbm_cmd_config(int argc, char **argv) {
    if (argc == 0) {
        printf("Usage: memora config <command> [args]\n\n");
        printf("Commands:\n");
...
static int cbm_kill_other_instances(void) {
    ...
    const char *argv[] = {"taskkill", "/F",       "/FI", "IMAGENAME eq memora.exe",
                          "/FI",      pid_filter, NULL};
    ...
    int killed = 0;
    pid_t self = getpid();
    FILE *fp = cbm_popen("pgrep -x memora", "r");
    if (!fp) {
        return 0;
    }
...
static int verify_download_checksum(const char *archive_path, const char *archive_name) {
    ...
    if (dl_base && dl_base[0]) {
        ...
        snprintf(checksum_url, sizeof(checksum_url), "%s",
                 "https://github.com/TakaraDasein/memora/releases/latest/download/"
                 "checksums.txt");
    }
...
static void install_copilot_durable_context(const char *home, const char *binary_path, bool force,
    ...
    snprintf(hook_path, sizeof(hook_path), "%s/hooks/memora.json", config_dir);
    snprintf(skills_dir, sizeof(skills_dir), "%s/skills", config_dir);
...
#ifdef _WIN32
    snprintf(binary_path, binary_path_size, "%s/.local/bin/memora.exe", home);
#else
    snprintf(binary_path, binary_path_size, "%s/.local/bin/memora", home);
#endif
}
...
static void install_editor_agent_configs(const cbm_detected_agents_t *agents, const char *home,
    ...
    if (agents->kilocode) {
        ...
        snprintf(ip, sizeof(ip), "%s/.config/kilo/rules/memora.md", home);
        ...
        if (!dry_run && !g_install_plan) {
            ...
            snprintf(legacy_cp, sizeof(legacy_cp),
                     ...
                     home);
#endif
            snprintf(legacy_ip, sizeof(legacy_ip), "%s/.kilocode/rules/memora.md",
                     home);
...
static void install_additional_agent_configs(const cbm_detected_agents_t *agents, const char *home,
    ...
    if (agents->cline) {
        ...
        snprintf(ip, sizeof(ip), "%s/rules/memora.md", cline_root);
...
static void cbm_detect_self_path(char *buf, size_t buf_sz, const char *home) {
    ...
    if (!buf[0]) {
#ifdef _WIN32
        snprintf(buf, buf_sz, "%s/.local/bin/memora.exe", home);
#else
        snprintf(buf, buf_sz, "%s/.local/bin/memora", home);
#endif
    }
}
...
char *cbm_build_install_plan_json(const char *home, const char *binary_path) {
    ...
    yyjson_mut_obj_add_str(doc, root, "next_safe_command", "memora install -y");
...
#ifdef _WIN32
    snprintf(bin_target, sizeof(bin_target), "%s/.local/bin/memora.exe", home);
#else
    snprintf(bin_target, sizeof(bin_target), "%s/.local/bin/memora", home);
#endif
...
static void uninstall_additional_agents(const cbm_detected_agents_t *agents, const char *home,
    ...
    if (agents->augment) {
        ...
        snprintf(coverage_hp, sizeof(coverage_hp), "%s/.augment/hooks/%s", home,
                 AUGMENT_COVERAGE_SCRIPT);
#ifdef _WIN32
        snprintf(binary_path, sizeof(binary_path), "%s/.local/bin/memora.exe", home);
#else
        snprintf(binary_path, sizeof(binary_path), "%s/.local/bin/memora", home);
#endif
...
#ifdef _WIN32
    snprintf(bin_path, sizeof(bin_path), "%s/.local/bin/memora.exe", home);
#else
    snprintf(bin_path, sizeof(bin_path), "%s/.local/bin/memora", home);
#endif
    struct stat st;
...
static void build_update_url(char *url, int url_sz, const char *os, const char *arch,
    ...
    if (!base_url || !base_url[0]) {
        base_url = "https://github.com/TakaraDasein/memora/releases/latest/download";
    }
    ...
    snprintf(url, url_sz, "%s/memora-%s%s-%s%s.%s", base_url, want_ui ? "ui-" : "", os,
             arch, portable, ext);
}
...
static int download_verify_install(const char *url, const char *ext, const char *os,
    ...
    const char *portable = (strcmp(os, "linux") == 0) ? "-portable" : "";
    snprintf(archive_name, sizeof(archive_name), "memora-%s%s-%s%s.%s",
...
static char *fetch_latest_tag(void) {
    FILE *fp = cbm_popen(
        "curl -sfI https://github.com/TakaraDasein/memora/releases/latest 2>/dev/null",
        "r");
    if (!fp) {
        return NULL;
    }
...
int cbm_cmd_update(int argc, char **argv) {
    ...
    printf("memora update (current: %s)\n\n", CBM_VERSION);
    ...
    if (dry_run) {
        ...
        printf("  target: %s/.local/bin/memora\n", home);
        printf("  variant: %s\n", variant_label);
        printf("  os/arch: %s/%s\n", os, arch);
...
#ifdef _WIN32
    snprintf(bin_dest, sizeof(bin_dest), "%s/.local/bin/memora.exe", home);
#else
    snprintf(bin_dest, sizeof(bin_dest), "%s/.local/bin/memora", home);
#endif
    char bin_dir[CLI_BUF_1K];
    snprintf(bin_dir, sizeof(bin_dir), "%s/.local/bin", home);
    cbm_mkdir_p(bin_dir, CLI_OCTAL_PERM);
...
int cbm_cli_print_tool_help(const char *tool_name) {
    ...
    printf("Usage:\n");
    printf("  memora cli %s --flag value [--flag2 value2 ...]\n", tool_name);
    printf("  memora cli %s --args-file <path-to-json>\n", tool_name);
    printf("  echo '<json>' | memora cli %s\n", tool_name);
    printf("  memora cli %s '<raw-json-args>'\n", tool_name);
