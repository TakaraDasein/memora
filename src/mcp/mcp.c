...
static char *cbm_mcp_initialize_response_for_profile(const char *params_json,
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "memora");
...
/* #1025: agents naturally pass the repo FOLDER name ("memora"),
...
static bool write_skip_logfile(const char *project, const cbm_file_error_t *errs, int count,
    ...
    (void)fprintf(f, "# memora index coverage report\n");
...
#define DEFAULT_AUTO_INDEX_LIMIT 50000
    ...
    if (!auto_index) {
        cbm_log_info("autoindex.skip", "reason", "disabled", "hint",
                     "run: memora config set auto_index true");
        return;
    }
...
#define UPDATE_CHECK_URL "https://api.github.com/repos/TakaraDasein/memora/releases/latest"
    ...
    if (tag_str) {
        ...
        if (cbm_compare_versions(tag_str, current) > 0) {
            snprintf(srv->update_notice, sizeof(srv->update_notice),
                     "Update available: %s -> %s -- run: memora update  |  "
                     "Enjoying memora? Please leave a star: "
