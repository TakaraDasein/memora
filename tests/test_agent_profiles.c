...
static const direct_dialect_expectation_t direct_dialects[] = {
    ...
    {CBM_GRAPH_DIALECT_COPILOT, "memora/check_index_coverage", "  - read\n",
    ...
    {CBM_GRAPH_DIALECT_QODER, "mcp__memora__check_index_coverage",
     "tools: Read,Grep,Glob,mcp__memora__", "Read,Grep"},
    {CBM_GRAPH_DIALECT_CODEBUDDY, "permissionMode: plan", "tools: Read,Grep,Glob,", "Read,Grep"},
    {CBM_GRAPH_DIALECT_FACTORY, "mcp__memora__check_index_coverage",
...
TEST(agent_profiles_tiers_encode_distinct_evidence_budgets) {
    ...
    int valid = scout && verify && audit && strstr(scout, "3-4 narrow graph calls") &&
                ...
                !strstr(scout, "mcp__memora__query_graph") &&
                !strstr(scout, "mcp__memora__detect_changes") &&
                strstr(verify, "default tier") && strstr(verify, "task-directed evidence") &&
                strstr(verify, "scope coverage before negative claims") &&
                strstr(verify, "mcp__memora__query_graph") &&
                strstr(verify, "mcp__memora__detect_changes") &&
                ...
                strstr(audit, "mcp__memora__query_graph") &&
                strstr(audit, "mcp__memora__detect_changes");
    free(scout);
    free(verify);
    free(audit);
    ASSERT_TRUE(valid);
    ...
    for (int dialect = 0; dialect < (int)CBM_GRAPH_DIALECT_COUNT; dialect++) {
        for (int tier = 0; tier < (int)CBM_GRAPH_TIER_COUNT; tier++) {
            ...
            int valid = strstr(profile, "parent agent must supply") &&
                        ...
                        !strstr(profile, "mcp__memora__") &&
                        !strstr(profile, "mcp_memora_") &&
...
TEST(agent_profiles_server_level_dialects_hard_enforce_read_only_tools) {
    ...
    ASSERT(strstr(qoder, "mcp__memora__check_index_coverage") != NULL);
    ASSERT(strstr(factory, "mcp__memora__check_index_coverage") != NULL);
    ASSERT(strstr(qoder, "mcpServers:") != NULL);
    ASSERT(strstr(qoder, "memora") != NULL);
...
TEST(agent_profiles_kiro_is_valid_json_and_escapes_binary_path) {
    ...
    yyjson_val *server = servers ? yyjson_obj_get(servers, "memora") : NULL;
    ...
    int valid = root && yyjson_is_obj(root) && command && yyjson_is_str(command) &&
                ...
                yyjson_is_arr(tools) &&
                strstr(profile, "@memora/check_index_coverage") != NULL;
    yyjson_doc_free(doc);
    free(profile);
    ASSERT_TRUE(valid);
    PASS();
