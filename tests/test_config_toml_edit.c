...
static const char *CTE_BEGIN = "# BEGIN memora";
static const char *CTE_END = "# END memora";
static const char *CTE_TABLE = "mcp_servers";
static const char *CTE_KEY = "name";
static const char *CTE_IDENTITY = "memora";
static const char *CTE_BODY = "name = \"memora\"\n"
                              "command = \"memora\"\n";
...
TEST(config_toml_codex_semantic_conflicts_fail_closed) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *codex_block = "[mcp_servers.memora]\ncommand = \"new\"\n";
    static const char *conflicts[] = {
        "[mcp_servers.\"memora\"]\ncommand = \"old\"\n",
        "[\"mcp_servers\".'memora']\ncommand = \"old\"\n",
        "mcp_servers.\"memora\".command = \"old\"\n",
        "[mcp_servers]\n\"memora\".command = \"old\"\n",
        ("[mcp_servers.memora]\ncommand = \"one\"\n"
         "[mcp_servers.\"memora\"]\ncommand = \"two\"\n"),
    };
...
TEST(config_toml_target_table_rejects_significant_nonassignments_byte_identically) {
    ...
    const char *malformed_regular = "[mcp_servers.memora]\n"
                                    "command = \"winner\"\n"
                                    "this is not an assignment\n"
                                    "[unrelated]\n"
                                    "keep = true\n";
    ...
    ASSERT_EQ(
        cbm_toml_remove_legacy_table(path, "mcp_servers.memora", CTE_BEGIN, CTE_END),
        -1);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
...
TEST(config_toml_vibe_reinstall_preserves_user_fields_and_descendants) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *desired = "name = \"memora\"\n"
                          "transport = \"stdio\"\n"
                          "command = \"new\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
    ASSERT_EQ(th_write_file(path, "[[mcp_servers]]\n"
                                  "name = \"memora\" # identity comment\n"
                                  "command = \"old\"\n"
...
TEST(config_toml_preserves_bom_crlf_and_handles_no_final_newline) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "# BEGIN memora\r\nowned = true\r\n"));
    ASSERT_EQ(cbm_toml_upsert_managed_block(path, CTE_BEGIN, CTE_END, "owned = true\n"), 0);
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ...
    ASSERT_EQ(th_write_file(path, "\xEF\xBB\xBF# BEGIN memora\r\n"
                                  "old = true\r\n"
                                  "# END memora\r\n"),
              0);
    ...
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT(memcmp(actual, "\xEF\xBB\xBF", 3U) == 0);
    ASSERT_NOT_NULL(strstr(actual, "command = \"memora\"\r\n"));
...
TEST(config_toml_managed_fresh_missing_file) {
    ...
    ASSERT_EQ(cte_read(path, actual, sizeof(actual)), 0);
    ASSERT_STR_EQ(actual, "# BEGIN memora\n"
                          "enabled = true\n"
                          "# END memora\n");
    th_cleanup(dir);
    PASS();
...
TEST(config_toml_managed_unbalanced_duplicate_fail_closed) {
    ...
    const char *duplicate = "# BEGIN memora\n"
                            "one = true\n"
                            "# END memora\n"
                            "# BEGIN memora\n"
                            "two = true\n"
...
TEST(config_toml_vibe_replace_target_preserves_comments_tables) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *replacement = "name = \"memora\"\n"
                              "command = \"/new/path\"\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
...
TEST(config_toml_vibe_owned_table_installs_idempotently_and_removes_exact_state) {
    ...
    char path[CTE_PATH_CAP];
    char actual[CTE_FILE_CAP];
    const char *canonical = "name = \"memora\"\n"
                            "transport = \"stdio\"\n"
                            "command = \"/opt/memora\"\n"
                            "args = []\n";
...
TEST(config_toml_vibe_ambiguous_target_fail_closed) {
    ...
    ASSERT_NOT_NULL(strstr(actual, "name = \"other\""));
    ASSERT_NOT_NULL(strstr(actual, "name = \"memora\""));
    th_cleanup(dir);
    PASS();
}
