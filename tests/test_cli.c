 ...
 *   - cmd/memora/cli_test.go (11 tests)
 *   - cmd/memora/install_test.go (24 tests)
 *   - cmd/memora/update_test.go (5 tests)
...
TEST(cli_legacy_skill_cleanup_rejects_links_and_user_content) {
    ...
    snprintf(monolithic_link, sizeof(monolithic_link), "%s/memora", skills_dir);
...
TEST(cli_remove_old_monolithic_skill) {
    ...
    char old_dir[1024];
    snprintf(old_dir, sizeof(old_dir), "%s/memora", skills_dir);
    test_mkdirp(old_dir);
...
TEST(cli_editor_mcp_install) {
    ...
    int rc = cbm_install_editor_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);

    const char *data = read_test_file(configpath);
    ASSERT_NOT_NULL(data);
    ASSERT(strstr(data, "mcpServers") != NULL);
    ASSERT(strstr(data, "\"memora\"") != NULL);
    ASSERT(strstr(data, "/usr/local/bin/memora") != NULL);
...
TEST(cli_editor_mcp_idempotent) {
    ...
    cbm_install_editor_mcp("/usr/local/bin/memora", configpath);
    int rc = cbm_install_editor_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
    ...
    const char *data = read_test_file(configpath);
    ASSERT_NOT_NULL(data);
    /* Count occurrences of "memora" (should be exactly 1 in mcpServers) */
    int count = 0;
    const char *p = data;
    while ((p = strstr(p, "\"memora\"")) != NULL) {
        count++;
        p += 20;
    }
...
TEST(cli_editor_mcp_uninstall) {
    ...
    cbm_install_editor_mcp("/usr/local/bin/memora", configpath);
    int rc = cbm_remove_editor_mcp_owned("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);

    const char *data = read_test_file(configpath);
    ASSERT_NOT_NULL(data);
    /* memora should be removed */
    ASSERT(strstr(data, "\"memora\"") == NULL);
...
TEST(cli_junie_mcp_install_issue651) {
    ...
    int rc = cbm_upsert_junie_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
    ...
    rc = cbm_upsert_junie_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
    ...
    rc = cbm_remove_junie_mcp_owned("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
    ...
    const char *partly_foreign =
        "{\"mcpServers\":{"
        "\"memora\":{\"command\":\"/usr/local/bin/memora\","
        "\"args\":[]},"
        "\"codebase-memory-scout\":{\"command\":\"/usr/local/bin/memora\","
...
TEST(cli_openclaw_mcp_install_uses_nested_servers) {
    ...
    int rc = cbm_install_openclaw_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_openclaw_mcp_preserves_valid_json5) {
    ...
    bool installed = data && strstr(data, "memora");
...
TEST(cli_openclaw_mcp_uninstall_uses_nested_servers) {
    ...
    ASSERT_EQ(cbm_install_openclaw_mcp("/usr/local/bin/memora", configpath), 0);
    ASSERT_EQ(cbm_remove_openclaw_mcp_owned("/usr/local/bin/memora", configpath), 0);
...
TEST(cli_openclaw_compaction_preserves_user_owned_section) {
    ...
    cbm_install_agent_configs(tmpdir, "/usr/local/bin/memora", false, false);
    char *installed = read_test_file_alloc(config_path);
    bool installed_owned =
        installed && strstr(installed, "Codebase Knowledge Graph (memora)");
    ...
    bool removed_owned =
        uninstalled && !strstr(uninstalled, "Codebase Knowledge Graph (memora)");
    free(uninstalled);
...
TEST(cli_openclaw_profile_uses_profile_state_and_default_workspace) {
    ...
    char *plan = cbm_build_install_plan_json(tmpdir, "/usr/local/bin/memora");
...
TEST(cli_openclaw_uninstall_removes_compaction_when_workspace_is_ambiguous) {
    ...
    write_test_file(config_path,
                    ...
                    "\"Codebase Knowledge Graph (memora)\"]}}}}\n");
    ...
    bool removed = after && !strstr(after, "Codebase Knowledge Graph (memora)");
...
TEST(cli_vscode_mcp_install) {
    ...
    int rc = cbm_install_vscode_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_vscode_mcp_uninstall) {
    ...
    cbm_install_vscode_mcp("/usr/local/bin/memora", configpath);
    int rc = cbm_remove_vscode_mcp_owned("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_vscode_profile_mcp_uninstall) {
...
#ifdef _WIN32
    snprintf(installed_binary, sizeof(installed_binary), "%s/.local/bin/memora.exe",
             tmpdir);
#else
    snprintf(installed_binary, sizeof(installed_binary), "%s/.local/bin/memora",
             tmpdir);
#endif
    ...
    bool removed = base && profile && !strstr(base, "memora") &&
                   !strstr(profile, "memora");
...
TEST(cli_zed_mcp_install) {
    ...
    int rc = cbm_install_zed_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_zed_mcp_preserves_settings) {
    ...
    cbm_install_zed_mcp("/usr/local/bin/memora", configpath);
...
TEST(cli_zed_mcp_uninstall) {
    ...
    cbm_install_zed_mcp("/usr/local/bin/memora", configpath);
    int rc = cbm_remove_zed_mcp_owned("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_extract_binary_from_targz) {
    ...
    int gz_len;
    unsigned char *gz =
        create_test_targz("memora-linux-amd64", (const unsigned char *)content,
...
TEST(cli_extract_binary_from_zip) {
    ...
    unsigned char *zip = create_test_zip_stored(
        "memora", (const unsigned char *)content, (int)strlen(content), &zip_len);
    ASSERT_NOT_NULL(zip);
...
TEST(cli_extract_binary_from_zip_path_traversal) {
    ...
    int zip_len = 0;
    unsigned char *zip =
        create_test_zip_stored("../../etc/memora", (const unsigned char *)content,
                               (int)strlen(content), &zip_len);
    ASSERT_NOT_NULL(zip);
...
TEST(cli_agent_install_reports_safe_editor_refusal) {
    ...
    char *saved_path = save_test_env("PATH");
    cbm_setenv("PATH", tmpdir, 1);
    int rc = cbm_install_agent_configs(tmpdir, "/usr/local/bin/memora", false, false);
    char *after = read_test_file_alloc(config_path);
...
TEST(cli_special_hook_failures_propagate_from_install_and_uninstall) {
    ...
    cbm_setenv("HOME", tmpdir, 1);
    cbm_setenv("PATH", tmpdir, 1);
    int install_rc = cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
    char *args[] = {"-n"};
...
TEST(cli_install_plan_receipt_no_mutation_issue388) {
    ...
    char *json = cbm_build_install_plan_json(tmpdir, "/usr/local/bin/memora");
...
TEST(cli_new_agent_configs_use_documented_schemas) {
    ...
    const char *binary = "/usr/local/bin/memora";
    cbm_install_agent_configs(tmpdir, binary, false, false);

    bool schemas_ok = true;
    const char *const hermes[] = {"mcp_servers:", "memora:", "command:", binary};
    ...
    const char *const standard_json[] = {"mcpServers", "memora", binary};
    ...
    const char *const copilot[] = {"mcpServers", "memora", "\"type\"", "local",
                                   binary};
    snprintf(path, sizeof(path), "%s/.copilot/mcp-config.json", tmpdir);
    ...
    snprintf(path, sizeof(path), "%s/.copilot/hooks/memora.json", tmpdir);
    schemas_ok = schemas_ok && test_file_contains_all(path, copilot_hooks, 9);

    const char *const factory[] = {"mcpServers", "memora", "stdio", binary};
    ...
    const char *const crush[] = {
        "\"mcp\"",       "memora", "stdio", binary, "\"options\"",
    ...
    const char *const goose[] = {
        "extensions:", "memora:", "type:", "stdio", "cmd:", binary};
#ifdef _WIN32
    ...
    const char *const vibe[] = {"[[mcp_servers]]", "name = \"memora\"",
...
TEST(cli_agent_reinstall_preserves_foreign_policy_entries) {
    ...
    cbm_setenv("HOME", tmpdir, 1);
    cbm_setenv("PATH", tmpdir, 1);
    int install_rc = cbm_install_agent_configs(tmpdir, "/new/memora", false, false);
    ...
    for (size_t i = 0U; i < sizeof(files) / sizeof(files[0]); i++) {
        ...
        if (i + 1U == sizeof(files) / sizeof(files[0])) {
            preserved = preserved && data && strstr(data, "/opt/user-tool") &&
                        ...
                        strstr(data, "Codebase Knowledge Graph (memora)");
        } else {
...
TEST(cli_durable_profiles_follow_current_vendor_paths) {
    ...
    char *plan = cbm_build_install_plan_json(tmpdir, "/opt/memora");
    ...
    const char *const claude_terms[] = {"name: codebase-memory",
                                        "mcpServers: [memora]",
                                        "mcp__memora__search_graph",
                                        "mcp__memora__check_index_coverage",
                                        "permissionMode: plan",
    ...
    const char *const codex_terms[] = {
        ...
        "[mcp_servers.memora]", "check_index_coverage"};
    ...
    files_ok = files_ok && profile &&
               !strstr(profile, "Use memora for read-only structural discovery");
    free(profile);
    ...
    yyjson_val *kiro_server =
        kiro_servers ? yyjson_obj_get(kiro_servers, "memora") : NULL;
    ...
    files_ok = files_ok && profile && kiro_root && yyjson_is_obj(kiro_root) && kiro_tools &&
               ...
               strcmp(yyjson_get_str(kiro_server_tool), "@memora/search_graph") == 0 &&
               kiro_servers && yyjson_is_obj(kiro_servers) && kiro_server &&
               yyjson_is_obj(kiro_server) && kiro_command && yyjson_is_str(kiro_command) &&
               strcmp(yyjson_get_str(kiro_command), "/opt/memora") == 0 && kiro_args &&
    ...
    const char *const vibe_agent_terms[] = {"agent_type = \"subagent\"",
                                            ...
                                            "\"memora_search_graph\"",
                                            "\"memora_get_code_snippet\"",
                                            "\"memora_check_index_coverage\""};
    files_ok = files_ok && test_file_contains_all(path, vibe_agent_terms, 6U);
    profile = read_test_file_alloc(path);
    files_ok = files_ok && profile && !strstr(profile, "memora_*") &&
               !strstr(profile, "delete_project") && !strstr(profile, "manage_adr");
    free(profile);
...
TEST(cli_cline_data_dir_only_redirects_data_state) {
    ...
    snprintf(rules, sizeof(rules), "%s/rules/memora.md", cline_root);
    ...
    snprintf(wrong_rules, sizeof(wrong_rules), "%s/rules/memora.md", data_dir);
...
TEST(cli_owned_durable_profiles_preserve_user_files) {
    ...
    int install_rc = cbm_install_agent_configs(tmpdir, "/opt/memora", true, false);
...
TEST(cli_tiered_codex_profiles_migrate_preserve_and_uninstall) {
    ...
    const char *legacy_verify =
        ...
        "Use memora for read-only structural discovery. Start with search_graph, "
...
TEST(cli_junie_current_durable_context_contract) {
    ...
    int reinstall_rc = cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
    ...
    int owned_reinstall_rc =
        cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
...
TEST(cli_rovo_installs_documented_global_memory) {
    ...
    int first_rc = cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
    char *first = read_test_file_alloc(memory_path);
    int second_rc = cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
...
#ifdef _WIN32
    snprintf(binary_path, sizeof(binary_path), "%s/.local/bin/memora.exe", tmpdir);
#else
    snprintf(binary_path, sizeof(binary_path), "%s/.local/bin/memora", tmpdir);
#endif
...
TEST(cli_detected_agent_summary_includes_registry_clients) {
    ...
    int install_rc =
        redirected ? cbm_install_agent_configs(tmpdir, "/opt/memora", false, true)
                   : -1;
    if (redirected) {
        fflush(stdout);
...
TEST(cli_agent_client_registry_routes_plan_install_and_uninstall) {
    ...
    bool qoder_settings_ok =
        ...
        yyjson_obj_get(qoder_servers, "memora") && session_hooks &&
    ...
    const char *const qoder_agent_terms[] = {"name: codebase-memory",
                                             "description:",
                                             "tools: Read,Grep,Glob,mcp__memora__",
                                             "mcp__memora__check_index_coverage",
                                             "search_graph",
                                             "trace_path"};
    const char *const graph_terms[] = {"codebase-memory", "search_graph", "trace_path"};
    ...
    durable_ok = durable_ok && qoder_agent_data && !strstr(qoder_agent_data, "Bash") &&
                 ...
                 strstr(qoder_agent_data, "- memora") &&
                 strstr(qoder_agent_data, "mcp__memora__check_index_coverage") &&
    ...
    struct stat state;
    bool mcp_ok = amazon_data && strstr(amazon_data, "memora") &&
                  strstr(amazon_data, binary_path) && roo_data &&
                  strstr(roo_data, "memora") && strstr(roo_data, binary_path) &&
                  cody_data && strstr(cody_data, "memora") &&
    ...
    bool qoder_owned_cleanup =
        qoder_data && (!qoder_servers || !yyjson_obj_get(qoder_servers, "memora")) &&
    ...
    bool registry_cleanup = amazon_data && strstr(amazon_data, "amazon") &&
                            !strstr(amazon_data, "memora") && roo_data &&
                            strstr(roo_data, "roo") && !strstr(roo_data, "memora") &&
                            cody_data && strstr(cody_data, "memora") &&
...
TEST(cli_registry_installs_gitlab_and_devin_lifecycle_context) {
    ...
    bool installed =
        ...
        test_file_contains_all(
            gitlab_mcp,
            (const char *const[]){"memora", binary_path, "\"type\": \"stdio\""}, 3U);
    free(gitlab_data);
    free(devin_data);
...
#endif
        (!gitlab_mcp_data || !strstr(gitlab_mcp_data, "memora"));
    bool devin_clean =
        devin_data && strstr(devin_data, "theme_mode") && !strstr(devin_data, "--dialect devin") &&
        !strstr(devin_data, "memora") && devin_agents_data &&
...
TEST(cli_devin_does_not_duplicate_owned_claude_session_start) {
    ...
    cbm_setenv("APPDATA", appdata, 1);
#endif
    int rc = cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
...
TEST(cli_registry_installs_codebuddy_bob_and_pochi_durable_context) {
    ...
    bool codebuddy_installed =
        test_file_contains_all(
            codebuddy_mcp, (const char *const[]){"mcpServers", "memora", binary_path},
            3U) &&
        ...
        test_file_contains_all(
            ...
            (const char *const[]){"permissionMode: plan",
                                  "tools: Read,Grep,Glob,mcp__memora__search_graph,",
                                  "mcp__memora__check_index_coverage",
                                  "skills: codebase-memory"},
            4U) &&
    ...
    bool bob_ide_mcp_installed = test_file_contains_all(
        bob_ide_mcp, (const char *const[]){"bob-ide", "memora", binary_path}, 3U);
    bool bob_shell_mcp_installed = test_file_contains_all(
        bob_shell_mcp, (const char *const[]){"memora", binary_path}, 2U);
    ...
    bool pochi_installed =
        test_file_contains_all(
            pochi_mcp, (const char *const[]){"\"mcp\"", "memora", binary_path}, 3U) &&
...
TEST(cli_gemini_installs_dedicated_graph_subagent) {
    ...
    bool content_ok = agent && strstr(agent, "name: codebase-memory") &&
                      ...
                      strstr(agent, "mcp_memora_search_graph") &&
                      strstr(agent, "mcp_memora_check_index_coverage") &&
                      !strstr(agent, "mcp_memora_delete_project");
    free(agent);
...
TEST(cli_fresh_cli_only_yaml_and_toml_agents_create_parent_dirs) {
    ...
    bool installed = test_file_contains_all(
        path, (const char *const[]){"mcp_servers:", "memora:"}, 2);
#ifdef _WIN32
    snprintf(path, sizeof(path), "%s/AppData/Roaming/Block/goose/config/config.yaml", tmpdir);
#else
    snprintf(path, sizeof(path), "%s/.config/goose/config.yaml", tmpdir);
    ...
    installed =
        installed && test_file_contains_all(
                         path, (const char *const[]){"extensions:", "memora:"}, 2);
    snprintf(path, sizeof(path), "%s/.vibe/config.toml", tmpdir);
    installed =
        installed && test_file_contains_all(
                         path, (const char *const[]){"[[mcp_servers]]", "memora"}, 2);
...
TEST(cli_augment_installs_session_context_and_subagent) {
    ...
    snprintf(bin_dir, sizeof(bin_dir), "%s/.local/bin", tmpdir);
#ifdef _WIN32
    snprintf(binary, sizeof(binary), "%s/memora.exe", bin_dir);
#else
    snprintf(binary, sizeof(binary), "%s/memora", bin_dir);
#endif
    ...
    bool settings_ok = settings && strstr(settings, "mcpServers") &&
                       strstr(settings, "memora") && strstr(settings, binary) &&
    ...
    bool removed = (!settings_after || (!strstr(settings_after, "memora") &&
...
TEST(cli_hook_ownership_requires_exact_command_identity) {
    ...
    const char *foreign =
        ...
        "\"type\":\"command\",\"command\":\"echo memora search_graph "
        "user-owned-gemini\"}]}]}}\n";
    ...
    bool install_preserved =
        ...
        test_count_substring(after_install, "memora search_graph") == 2U;
    free(after_install);
    ...
    bool remove_preserved =
        ...
        test_count_substring(after_remove, "memora search_graph") == 1U;
    free(after_remove);
...
TEST(cli_gemini_hook_upgrade_migrates_released_exact_commands) {
    ...
    static const char *const legacy_before_commands[] = {
        "echo 'Reminder: prefer memora search_graph/trace_path/"
        "get_code_snippet over grep/file search for code discovery.' >&2",
        "echo 'Reminder: prefer memora search_graph/trace_call_path/"
        "get_code_snippet over grep/file search for code discovery.' >&2",
    };
    ...
    for (size_t i = 0U; i < sizeof(legacy_before_commands) / sizeof(legacy_before_commands[0]);
        ...
        int written = snprintf(
            ...
            "memora (search_graph, trace_path, get_code_snippet, query_graph, "
...
TEST(cli_uninstall_preserves_hook_script_with_modified_binary) {
    ...
    char binary[640];
#ifdef _WIN32
    snprintf(binary, sizeof(binary), "%s/.local/bin/memora.exe", tmpdir);
#else
    snprintf(binary, sizeof(binary), "%s/.local/bin/memora", tmpdir);
#endif
    int install_rc = cbm_install_agent_configs(tmpdir, binary, false, false);
...
TEST(cli_claude_lifecycle_hooks_delegate_to_augmenter) {
    ...
    const char *binary = "/opt/codebase memory/bin/memora";
    cbm_install_agent_configs(tmpdir, binary, false, false);
...
TEST(cli_copilot_install_preserves_foreign_named_manifest) {
    ...
    snprintf(manifest_path, sizeof(manifest_path), "%s/memora.json", hooks_dir);
    test_mkdirp(hooks_dir);
...
TEST(cli_vscode_only_installs_copilot_durable_context) {
    ...
    snprintf(hook_path, sizeof(hook_path), "%s/.copilot/hooks/memora.json", tmpdir);
...
TEST(cli_lifecycle_hooks_preserve_foreign_substring_commands) {
    ...
    const char *qwen_foreign =
        ...
        "\"type\":\"command\",\"command\":\"/opt/user-memora-wrapper "
        "--keep-session\"}]}],"
        "\"SubagentStart\":[{\"matcher\":\"*\",\"hooks\":[{\"type\":\"command\","
        "\"command\":\"/opt/user-memora-wrapper --keep-subagent\"}]}]}}\n";
    const char *factory_foreign =
        "{\"hooks\":{\"SessionStart\":[{\"hooks\":[{\"type\":\"command\","
        "\"command\":\"/opt/user-memora-wrapper --keep-factory\"}]}]}}\n";
...
TEST(cli_read_only_agents_do_not_receive_mutating_mcp_server) {
    ...
    bool qoder_confined = qoder && strstr(qoder, "mcpServers:") &&
                          strstr(qoder, "- memora") &&
                          strstr(qoder, "mcp__memora__search_graph") &&
    ...
    bool kiro_confined =
        ...
        strstr(kiro, "@memora/search_graph") && strstr(kiro, "--tool-profile") &&
        strstr(kiro, "analysis") && strstr(kiro, "check_index_coverage") &&
        !strstr(kiro, "\"@memora\"") && !strstr(kiro, "delete_project") &&
...
TEST(cli_mcp_installers_preserve_foreign_same_name_entries) {
    ...
    const char *foreign_json =
        "{\"mcpServers\":{\"memora\":{\"command\":"
        "\"/opt/custom/memora\",\"args\":[]}},\"theme\":\"dark\"}\n";
    const char *foreign_toml = "[mcp_servers.memora]\n"
                               "command = \"/opt/user-tool\"\n"
                               "args = [\"--private\"]\n"
                               "env = { KEEP = \"yes\" }\n";

    write_test_file(json_path, foreign_json);
    int json_install_rc = cbm_install_editor_mcp("/opt/memora", json_path);
    ...
    int toml_install_rc = cbm_upsert_codex_mcp("/opt/memora", toml_path);
...
TEST(cli_installer_rejects_symlinked_agent_roots) {
    ...
    cbm_setenv("HOME", tmpdir, 1);
    cbm_setenv("PATH", tmpdir, 1);
    (void)cbm_install_agent_configs(tmpdir, "/opt/memora", false, false);
...
TEST(cli_claude_hook_scripts_shell_quote_binary_path) {
    ...
    cbm_unsetenv("CLAUDE_CONFIG_DIR");
    cbm_unsetenv("COPILOT_HOME");
    const char *binary = "/opt/$(touch cbm-hook-pwned)/it's memora";
    cbm_install_agent_configs(tmpdir, binary, false, false);
    ...
    char manifest_path[640];
    snprintf(manifest_path, sizeof(manifest_path), "%s/hooks/memora.json",
             copilot_dir);
...
TEST(cli_codex_migrates_to_single_hook_representation) {
    ...
    bool migrated = toml && !strstr(toml, "memora SessionStart") && hooks &&
...
static const char test_released_session_hook_script[] =
    "#!/usr/bin/env bash\n"
    "# SessionStart hook: remind agent to use memora tools.\n"
    "# Installed by memora. Fires on startup/resume/clear/compact.\n"
    "cat << 'REMINDER'\n"
    "CRITICAL - Code Discovery Protocol:\n"
    "1. ALWAYS use memora tools FIRST for ANY code exploration:\n"
...
static const char test_released_subagent_hook_script[] =
    "#!/usr/bin/env bash\n"
    "# SubagentStart hook: tell subagents to use memora tools.\n"
    "# Installed by memora. Fires when any subagent is spawned.\n"
    "# SubagentStart injects context via JSON additionalContext, not plain stdout.\n"
    "cat << 'REMINDER'\n"
    "{\"hookSpecificOutput\":{\"hookEventName\":\"SubagentStart\","
    "\"additionalContext\":\"Code discovery: prefer memora tools "
...
static bool test_build_released_gate_hook_script(const char *binary_path, char *script,
    ...
    int written = snprintf(script, script_size,
                           "#!/usr/bin/env bash\n"
                           "# memora search augmenter (Claude Code PreToolUse).\n"
...
TEST(cli_upgrade_migrates_released_claude_hook_scripts) {
    ...
    ASSERT_TRUE(test_build_released_gate_hook_script("/opt/memora", legacy_gate,
                                                     sizeof(legacy_gate)));
...
TEST(cli_codex_legacy_migration_rejects_linked_config) {
    ...
    int rc = cbm_upsert_codex_mcp("/usr/local/bin/memora", config);
...
TEST(cli_upsert_codex_mcp_fresh) {
    ...
    int rc = cbm_upsert_codex_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);

    const char *data = read_test_file(configpath);
    ASSERT_NOT_NULL(data);
    ASSERT(strstr(data, "[mcp_servers.memora]") != NULL);
    ASSERT(strstr(data, "/usr/local/bin/memora") != NULL);
...
TEST(cli_upsert_codex_mcp_escapes_windows_path) {
    ...
    char configpath[512];
    snprintf(configpath, sizeof(configpath), "%s/config.toml", tmpdir);
    const char *binary = "C:\\Users\\Martin Vogel\\bin\\memora.exe";
...
TEST(cli_upsert_codex_mcp_replace) {
    ...
    char configpath[512];
    snprintf(configpath, sizeof(configpath), "%s/config.toml", tmpdir);
    write_test_file(configpath, "[mcp_servers.memora]\n"
                                "command = \"/old/path/memora\"\n"
                                "\n"
                                "[other_setting]\nfoo = \"bar\"\n");

    int rc = cbm_upsert_codex_mcp("/new/path/memora", configpath);
    ASSERT_EQ(rc, 0);
    ...
    ASSERT(strstr(data, "/new/path/memora") != NULL);
...
TEST(cli_zed_mcp_preserves_jsonc_comments) {
    ...
    bool preserved = data && strstr(data, "preserve the user's Zed setting") &&
                     strstr(data, "Ayu Dark") && strstr(data, "memora");
    free(data);
    test_rmdir_r(tmpdir);
    if (rc != 0 || !preserved)
...
TEST(cli_upsert_opencode_mcp_fresh) {
    ...
    int rc = cbm_upsert_opencode_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_upsert_antigravity_mcp_fresh) {
    ...
    int rc = cbm_upsert_antigravity_mcp("/usr/local/bin/memora", configpath);
    ASSERT_EQ(rc, 0);
...
TEST(cli_upsert_antigravity_mcp_replace) {
    ...
    char configpath[512];
    snprintf(configpath, sizeof(configpath), "%s/mcp_config.json", tmpdir);
    write_test_file(configpath, "{\"mcpServers\":{\"memora\":{"
                                "\"command\":\"memora\"}}}");

    int rc = cbm_upsert_antigravity_mcp("/new/path/memora", configpath);
    ASSERT_EQ(rc, 0);

    const char *data = read_test_file(configpath);
    ASSERT_NOT_NULL(data);
    ASSERT(strstr(data, "\"command\":\"memora\"") == NULL);
    ASSERT(strstr(data, "/new/path/memora") != NULL);
...
TEST(cli_aider_instructions_are_cli_form_issue1032) {
    ...
    ASSERT(strstr(content, "memora cli search_graph") != NULL);
    ASSERT(strstr(content, "memora cli trace_path") != NULL);
    ASSERT(strstr(content, "memora cli index_repository") != NULL);
...
TEST(cli_upsert_instructions_fresh) {
    ...
    ASSERT_NOT_NULL(data);
    ASSERT(strstr(data, "<!-- memora:start -->") != NULL);
    ASSERT(strstr(data, "<!-- memora:end -->") != NULL);
...
TEST(cli_upsert_instructions_no_duplicate) {
    ...
    int count = 0;
    const char *p = data;
    while ((p = strstr(p, "memora:start")) != NULL) {
        count++;
        p += 25;
    }
    ASSERT_EQ(count, 1);
...
TEST(cli_agent_instructions_content) {
    ...
    ASSERT(strstr(instr, "## Codebase Knowledge Graph (memora)\n") != NULL);
    PASS();
}
...
TEST(cli_qwen_windows_hook_command_uses_powershell_schema) {
    ...
    char shell[32];
    int rc =
        cbm_build_qwen_hook_command_for_testing("C:\\Program Files\\memora.exe", true,
    ...
    ASSERT_EQ(cbm_upsert_qwen_lifecycle_hooks_for_testing(
                  settings, "C:\\Program Files\\memora.exe", true),
              0);
...
TEST(cli_windows_optional_hooks_require_a_documented_shell) {
    ...
    char command[1024];
    char shell[32];
    ASSERT_EQ(cbm_build_qoder_hook_command_for_testing("C:\\Program Files\\memora.exe",
...
TEST(cli_hook_gate_script_no_predictable_tmp_issue384) {
    ...
    cbm_install_hook_gate_script(tmpdir, "/usr/local/bin/memora");
...
TEST(cli_hook_scripts_platform_shape_issue929) {
    ...
    ASSERT_TRUE(cbm_install_hook_gate_script(tmpdir, "/usr/local/bin/memora"));
...
TEST(cli_tool_hooks_preserve_foreign_same_matcher) {
    ...
    bool installed = claude && strstr(claude, "user-claude-tool-hook") &&
                     ...
                     strstr(gemini, "memora search_graph");
    free(claude);
    free(gemini);
...
TEST(cli_upsert_gemini_hook_replace) {
    ...
    write_test_file(
        ...
        "\"hooks\":[{\"type\":\"command\","
        "\"command\":\"echo 'Reminder: prefer memora "
