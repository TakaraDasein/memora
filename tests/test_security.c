...
TEST(shell_accepts_clean_path) {
    ASSERT_TRUE(cbm_validate_shell_arg("/home/user/.local/bin/memora"));
    PASS();
}
 ...
 *  filter "IMAGENAME eq memora.exe" was passed as three
...
TEST(cmdline_taskkill_filter_is_single_quoted_token) {
    ...
    const char *argv[] = {"taskkill", "/FI", "IMAGENAME eq memora.exe", NULL};
    ASSERT_CMDLINE(argv, L"taskkill /FI \"IMAGENAME eq memora.exe\"");
    PASS();
}
...
TEST(cmdline_single_arg_no_trailing_space) {
    const char *argv[] = {"memora.exe", NULL};
    ASSERT_CMDLINE(argv, L"memora.exe");
    PASS();
}
